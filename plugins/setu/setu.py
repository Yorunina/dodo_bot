...
class setu:
    def find_setu(msg, r_18 = False):
        ...
        #建立消息对象
        msg.send(content = None, image_url = url)
        #关闭数据库服务
        conn.close()
        return
        ...
        #用于异步调用的函数
    def send_setu(msg):
        setu.find_setu(msg, False)
        return
    def send_setub(msg):
        setu.find_setu(msg, True)
        return

class plugin_load:
#匹配类型：私聊、匹配依据：setu、调用函数：send_setu、匹配方式：前缀匹配：prefix、匹配优先级：50（越大越优先）
    def __init__(self):
        api.match_update('group', 'setu', setu.send_setu, 'pre', 50)
        api.match_update('group', 'setub', setu.send_setub, 'pre', 50)
        return
